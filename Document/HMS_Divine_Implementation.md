...
- `spawnArmoryWave(guild)` 窶・checks: battle active, armory not secured, players present, no active pending wave. Creates `PendingEncounter` rows with `wave_id` (monotonic counter from `global.arb_armory_wave_counter`) and posts fight buttons to armory channel. **Enemy targeting:** players are shuffled, then enemies are assigned round-robin (`shuffledPlayers[i % shuffledPlayers.length]`), so each enemy in a wave can target a different player. **No longer called by `spawnEncounters()` or any cron job** 窶・driven entirely by `setTimeout`.
...
**Loss path retargeting** (`interactionCreate.js`): On player loss (win = false), after resetting the encounter to `pending`, the handler queries `CharacterBase` for other players in the same zone (excluding the fighter). If others are present, the encounter is retargeted to a random one and a weakened embed is reposted mentioning them. If no others are present and the player is not KO'd, the weakened embed reposts to the same player. If no others and the player is KO'd, no embed is reposted 窶・the encounter remains `pending` but unannounced, and is cancelled by `locationUtility.js` when the player is moved to living quarters.
